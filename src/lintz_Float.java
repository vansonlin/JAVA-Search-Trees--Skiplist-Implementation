...
//            System.out.println("Height = " + rbtree.getHeight());
    ...
    public void go(String[] args){
        ...
        try {
            ...
            List<List<Float>> dataList = readFile(); // read file, throw exception if the file not in correct format
            ...
            for (List<Float> s:dataList) {
                Float request = s.get(0);
                Float key = s.get(1);

                if (request == 1.){ // if 1, perform insertion or search depending on whether the key exists
     ...
     **/
    private List<List<Float>> readFile() throws Exception{
        // read file
        ...
        while ((line = br.readLine()) != null) {
            if (line.length() > 1){
                List<Float> dataLine = new ArrayList<Float>();
...
//                if (s.length == 2 & (s[0].equals("1") || s[0].equals("0"))){ // only two field
                if (s[0].equals("1") || s[0].equals("0")){ // multiple field
                    dataLine.add(Float.parseFloat(s[0]));
                    dataLine.add(Float.parseFloat(s[1]));
